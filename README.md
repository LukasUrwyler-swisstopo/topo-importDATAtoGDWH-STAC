...
Die Korrekturwerte sind fix im Skript hinterlegt (kein GUI-/CLI-Parameter mehr):

- **Als "falsch" erkannte 255,255,255-Gruppen** (Überstrahlung/Gletscher): fixe Verschiebung **−1** (255 → 254).
- **Schatten-Puffer-Korrektur bei NoData 0,0,0** (nahe-schwarze Pixel, R,G,B ≤ 5): statt einer einzigen scharfen Anhebung bei exakt 0,0,0 werden auch die nahe-schwarzen Nachbarpixel gestuft angehoben, je nach Tiefe des Schwarz-Einbruchs (min(R,G,B) = 0 → +5, 1 → +4, 2 → +3, 3 → +2, 4 → +1, 5 → +1). Das ersetzt die frühere flache Anhebung der als "falsch" erkannten 0,0,0-Gruppen und vermeidet einen unnatürlichen Sprung an der Kante der angehobenen Zone (ohne Pufferung bliebe der direkte Nachbar eines auf 5,5,5 angehobenen Pixels z.B. bei 1,2,2 stehen). Echte NoData-Gruppen (laut Klassifikation) bleiben dabei exakt 0,0,0, damit die wertbasierte NoData-Erkennung nicht bricht.

### Historische 255er-NoData-DOPs: Pixelwerte auf 0,0,0 normalisieren

Wählt man im GUI-Dropdown `NoData der Quelldaten` = `255 255 255` (GDS `SB_DOP`), werden zusätzlich alle als **echtes** NoData erkannten Pixel (nicht die "falschen" Gruppen von oben) direkt im Raster von 255,255,255 auf 0,0,0 umgeschrieben – im selben Lese-/Schreibdurchgang, der die Flag Mask ohnehin schon berechnet, also ohne zusätzlichen I/O. Ohne Zusatzschritt wären danach ganz normale, sehr dunkle Schattenpixel in den Nutzdaten (bereits nahe 0,0,0, unabhängig vom NoData-Perimeter) wertidentisch mit echtem NoData – manche Software liest den NoData-Tag wertbasiert statt nur über die Flag Mask und würde diese Schattenpixel dann fälschlich als NoData/transparent behandeln. Deshalb läuft die Korrektur in dieser Reihenfolge ab: zuerst die fixe −1-Verschiebung der falschen 255er-Gruppen, dann die Schatten-Puffer-Korrektur (siehe oben) über die ganze Kachel, erst danach der eigentliche NoData-Wechsel 255 → 0.
